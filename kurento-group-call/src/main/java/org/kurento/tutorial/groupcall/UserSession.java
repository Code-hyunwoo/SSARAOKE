...
public class UserSession implements Closeable {
  ...
  public UserSession(final String name, String roomName, final WebSocketSession session,
    ...
    // 파이프라인을 통해 WebRtcEndpoint 생성
    this.outgoingMedia = new WebRtcEndpoint.Builder(pipeline).build();

    // IceCandidate : WebRTC가 원격장치와 통신을 하기 위해 요구되는 프로토콜과 라우팅에 대해 알려줌
    // ICE(Interactive Connectivity Establishment)는 두 단말이 서로 통신할 수 있는 최적의 경로를 찾을 수 있도록 도와주는 프레임워크임
  ...
  public void receiveVideoFrom(UserSession sender, String sdpOffer) throws IOException {
    ...
    // 위에서 저장한 것들을 textMessage로 보냄
    this.sendMessage(scParams);
    log.debug("gather candidates");
    this.getEndpointForUser(sender).gatherCandidates();
  }

  // 해당 유저의 앤드포인트를 주는 getter
